...
function render(ctx) {
    ...
    if (eatKey(TOGGLE_CLEAR)) {
        g_doClear = !g_doClear;
        console.log("TOGGLE_CLEAR - " + g_doClear);
    }
    if (eatKey(TOGGLE_BOX)) {
        g_doBox = !g_doBox;
        console.log("TOGGLE_BOX - " + g_doBox);
    }
    if (eatKey(TOGGLE_UNDO_BOX)) {
        g_undoBox = !g_undoBox;
        console.log("TOGGLE_UNDO_BOX - " + g_undoBox);
    }
    if (eatKey(TOGGLE_FLIPFLOP)) {
        g_doFlipFlop = !g_doFlipFlop;
        console.log("TOGGLE_FLIPFLOP - " + g_doFlipFlop);
    }
    if (eatKey(TOGGLE_RENDER)) {
        g_doRender = !g_doRender;
        console.log("TOGGLE_RENDER - " + g_doRender);
    }
